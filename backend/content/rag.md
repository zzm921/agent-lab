...
本框架把 RAG 做成**多方案可选框架**：同一份内置语料（虚构「科创公司员工行政、考勤、福利与差旅全管理制度」）按方案写入各自独立的 Qdrant 集合（`knowledge_{scheme_id}`），侧边栏可随时切换方案，直观对比检索与回答差异。当前**首期落地 naive 方案**，advanced / modular / graph / agentic 在同一框架上逐步扩展。

- **naive（朴素 RAG）**：固定切块 + 纯稠密向量语义检索（DashScope `text-embedding-v3`），最简基线——详见 [naive-rag.md](naive-rag.md)；
- **advanced（高级 RAG，规划中）**：混合检索（稠密 + 稀疏 `text-sparse-embedding-v1`，Qdrant 内置 RRF 融合），补足关键词盲区——详见 [advanced-rag.md](advanced-rag.md)。

存储层以 `StoreBackend` 抽象解耦，Qdrant 为首个实现（ES 预留接口）；未配置 Qdrant 时回退内存检索，离线/测试可跑通。
