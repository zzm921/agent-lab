...
<template>
  <div class="rounded-xl border px-3 py-2 text-xs" :class="STATUS[entry.status].cls">
    <!-- 头部行：点击展开/收起 -->
    <button type="button" class="flex w-full cursor-pointer items-center justify-between gap-2 text-left" @click="toggle">
      ...
      <span class="flex shrink-0 items-center gap-2">
        <span>{{ STATUS[entry.status].label }}</span>
        <!-- 重试徽标：n/m + 指数退避秒数 -->
        <span
          v-if="entry.retryCount"
          class="rounded bg-amber-500/20 px-1.5 py-0.5 font-mono text-amber-300"
        ...
        <span class="font-mono text-slate-400 transition-transform" :class="{ 'rotate-90': expanded }">▶</span>
      </span>
    </button>
