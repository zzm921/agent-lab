...
const cardSections = computed(() => {
  ...
  if (tag.groups && tag.groups.length) {
    return tag.groups.map((g) => ({ key: `${tag.id}:${g.title}`, title: g.title, cards: g.cards }))
  }
  return [{ key: tag.id, title: null, cards: tag.cards }]
})

/** 大标签颜色：按分类 id 取色，未知回退主题主色 */
function catColor(id: string) {
  return CATEGORY_COLORS[id] ?? '#7c5cff'
}

/** 大标签标题：按分类 id 反查 tags 清单 */
function catTitle(id: string) {
  return tags.value.find((t) => t.id === id)?.title ?? id
}
      ...
      <section class="grid items-center gap-10 py-16 md:grid-cols-2 md:py-24">
        <div>
          ...
          <div class="mt-8 flex flex-wrap items-center gap-4">
            <a
              href="https://github.com/zzm921/agent-lab"
              target="_blank"
              rel="noopener noreferrer"
              class="group inline-flex items-center gap-2.5 rounded-xl bg-gradient-to-r from-indigo-500 to-fuchsia-500 px-5 py-2.5 text-sm font-medium text-white shadow-lg shadow-indigo-500/25 transition hover:brightness-110"
            >
              ...
              <svg class="h-4 w-4 transition group-hover:translate-x-0.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" :d="iconPath('arrowRight')" />
              </svg>
            </a>
            <span class="flex items-center gap-1.5 text-xs text-slate-500">
              <svg class="h-3.5 w-3.5 text-amber-400/90" fill="currentColor" viewBox="0 0 24 24">
        ...
        <template v-else>
          ...
          >
            ...
            <div class="mt-3 flex flex-wrap items-center gap-1.5">
              <span
                v-for="cid in cap.categoryIds"
                :key="cid"
                class="rounded-md px-2 py-0.5 text-[10px] font-semibold"
                :style="{ background: `${catColor(cid)}22`, color: catColor(cid) }"
              >
                {{ catTitle(cid) }}
              </span>
            </div>
            <div class="mt-2 flex flex-wrap gap-1">
              <span
                v-for="t in cap.tags"
                :key="t"
                class="rounded border border-white/10 bg-white/5 px-1.5 py-0.5 text-[10px] text-slate-500"
              >
                {{ t }}
              </span>
            </div>
      ...
      >
        ...
        <div class="mb-6">
          ...
          <div class="mt-3 flex flex-wrap items-center gap-2">
            ...
            <span
              v-for="cid in detailCap.categoryIds"
              :key="cid"
